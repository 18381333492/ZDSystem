...
namespace ADSystem.API.HWServer
{
    ...
    /// PaPayNotify 的摘要说明
    /// </summary>
    public class PaPayNotify : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                logger.Info("----------------平安银行支付回调开始---------------------");
                byte[] bNetStream = context.Request.BinaryRead(context.Request.ContentLength);//获取平安银行异步回调的请求数据的字节流
                ...
                {//获取支付Key为空
                    logger.Info("平安银行支付APIKey为空,导致签名验证失败");
                }
                //验证支付签名
