...
var messagePop = {
    ...
    dealMsgBox: "#dealMsg", //显示未处理的消息数量的box
    readMsgBox: "#readMsg", //显示未读的消息数量的box
    ...
    initMessagePop: function () {
        ...
        $(this.dealMsgBox + "," + this.readMsgBox).click(function (event) {
            WindowForm.AppendTab("我的消息", "/programecenter/usermessage", event);
        });
        this.fillMsgBox();
    },
    //弹出在屏幕中间
    ...
    //构造弹窗的内容HTML
    buildMsgContent: function (_dealingNum, _readingNum) {
        var contentHTML = "<p id='{0}'>"
            + "<span class='hand' onclick='WindowForm.AppendTab(\"我的消息\", \"/programecenter/usermessage\",event);'>待处理消息：<a>{1}</a></span><br/>"
            + "<span class='hand' onclick='WindowForm.AppendTab(\"我的消息\", \"/programecenter/usermessage\",event);'>未读的消息：<a>{2}</a></span>"
            + "</p>"; //" + this.popMsgWrapper + "
        if (_dealingNum > 0 || _readingNum > 0) return this.formatStr(contentHTML, this.popMsgWrapper, _dealingNum, _readingNum);
        else return null;
    },
    //显示并设置消息箱
    showAndSetMsgBox: function (_dealingNum, _readingNum) {
        //待处理消息
        if (_dealingNum > 0) $(this.dealMsgBox).html("&nbsp;" + _dealingNum);
        else $(this.dealMsgBox).html("");
        //未读消息
        if (_readingNum > 0) $(this.readMsgBox).html("&nbsp;" + _readingNum);
        else $(this.readMsgBox).html("");

        var _msg = this.buildMsgContent(_dealingNum, _readingNum);
        if (_msg && this.isPop) this.popToRightCorner(_msg);
    },
    //填充消息窗
    fillMsgBox: function () {
        ...
        var that = this;
        $.ajax({
            url: "/STUserAjax/GetMsgSituation/?" + Math.random(),
            success: function (data) {
                ...
                var _dealingNum = _data.DealingNum; //待处理消息数
                var _readingNum = _data.ReadingNum; //未读
                var _reqestTime = _data.ReqestTime; //重复获取消息时间间隔
                var _recvType = _data.RecvType; //提示类型

                if (!that.isNum(_dealingNum, _readingNum, _reqestTime, _recvType)) { return; }
                if (_reqestTime == -1) { return; } //没有该用户，不用再获取它的消息

                that.isPop = _recvType;
                that.showAndSetMsgBox(_dealingNum, _readingNum);
